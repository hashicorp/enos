...
func resDiags(res *pb.Operation_Response) []*pb.Diagnostic {
	return Concat(
		...
		res.GetCheck().GetGenerate().GetDiagnostics(),
		...
		res.GetLaunch().GetGenerate().GetDiagnostics(),
		...
		res.GetRun().GetGenerate().GetDiagnostics(),
		...
		res.GetDestroy().GetGenerate().GetDiagnostics(),
